...
public class Gau_MH_RayTracing_Multi : MonoBehaviour
{
    ...
    {
        public string AP;
        public float azimuthMu;
        public float azimuthSigma;
        public float elevationMu;
        public float elevationSigma;
    }
    // public string csvFileName = "AP_Gaussian_Params3.csv"; 
    private static List<APParams> apList = new List<APParams>();

    private float proposalStepAz = 0.2f;
    private float proposalStepEl = 0.2f;
    public int mcmcIterations = 1000;
    public float sigmaAz = 0.32f;
    public float sigmaEl = 1f;
    ...
    void Start()
    {
        outputFile1 = Path.Combine(basePath, "EstimateByVoxel_" + "Gau_MH_" + sigmaAz + "_" + sigmaEl + "_" + raysPerBaseStation + "_" + maxReflections + ".csv");
    ...
    void InitializeRays()
    {
        ...
        for (int i = 0; i < baseStations.Length; i++)
        {
            ...
            {
                // Step 1: 转为 azimuth/elevation
                (float az, float el) = samples[j];  // az/el 单位是弧度
                // Step 2: 加上基站朝向（假设 APyaw[i] 是度）
    ...
    {
        List<(float az, float el)> samples = new List<(float az, float el)>(); // ✅ 初始化空列表
        float az_curr = (azimuths[currentMeasureIndex][baseIndex] ?? 0f);
        float el_curr = (elevations[currentMeasureIndex][baseIndex] ?? 0f);
        float currentProb = PosteriorAzEl(az_curr, el_curr, baseIndex);
        int accepted = 0;
        for (int i = 0; i < mcmcIterations; i++)
        {
            // 提议角度，加入小扰动
            float az_prop = az_curr + GaussianRandom(0f, proposalStepAz);
            float el_prop = el_curr + GaussianRandom(0f, proposalStepEl);
    ...
    // 假设 az 和 el 独立高斯分布
    float PosteriorAzEl(float az, float el, int apIndex)
    {
        float muAz = (azimuths[currentMeasureIndex][apIndex] ?? 0f);
        float muEl = (elevations[currentMeasureIndex][apIndex] ?? 0f);

        float pAz = GaussianPDF(az, muAz, sigmaAz);
        float pEl = GaussianPDF(el, muEl, sigmaEl);
    ...
    {
        float diff = x - mu;
        return (1f / (Mathf.Sqrt(2f * Mathf.PI) * sigma)) * Mathf.Exp(-0.5f * diff * diff / (sigma * sigma));
    }
    ...
    //         {
    //             AP = tokens[0],
    //             azimuthMu = float.Parse(tokens[1]),
    //             azimuthSigma = float.Parse(tokens[2]),
    //             elevationMu = float.Parse(tokens[3]),
    //             elevationSigma = float.Parse(tokens[4])
    //         };
    //         apList.Add(ap);
    //     }
