...
public class TestRayTracing : MonoBehaviour
{
    ...
    public float test_std_azi = 0.1f;
    public float test_std_ele = 0.1f;
    void Start()
    {
        outputFile1 = Path.Combine(basePath, "EstimateByVoxel_" + "Gau_" + raysPerBaseStation + "_" + maxReflections + ".csv");
    ...
    void InitializeRays()
    {
        ...
        for (int i = 0; i < baseStations.Length; i++)
        {
			...
			{
				// float azi = GaussianRandom(apList[i].Azimuth_Gauss_Mean_rad, apList[i].Azimuth_Gauss_Std_rad);
				// float ele = GaussianRandom(apList[i].Elevation_Gauss_Mean_rad, apList[i].Elevation_Gauss_Std_rad);
				// Vector3 dir = SphericalToCartesian(azi, ele);

				// Vector3 gtPos = ground_truth[currentMeasureIndex];
				// Vector3 dir = (gtPos - pos).normalized;
				// (float azi, float ele) = CartesianToSpherical(dir);
				// float azi = (azimuths[currentMeasureIndex][i] ?? 0f)+ Mathf.Deg2Rad*APyaw[i] + GaussianRandom(0,test_std_azi);
				// float ele = (elevations[currentMeasureIndex][i] ?? 0f)+ GaussianRandom(0,test_std_ele);
				// Vector3 dir = SphericalToCartesian(azi,ele);

				float az = UnityEngine.Random.Range(0f, 360f);
				float el = UnityEngine.Random.Range(-45f, 45f);  // 或更宽
				Vector3 dir = SphericalToCartesian(az, el);
				...
				if (RayHitsGT(ray, maxReflections, ground_truth[currentMeasureIndex]))
				{
					Debug.Log(az + ',' + el);
				}
	...
	bool RayHitsGT(Ray ray, int maxReflections, Vector3 gtPos)
	{
		...
		for (int i = 0; i <= maxReflections; i++)
		{
			...
			{
				// 判断是否命中 GT
				if (Vector3.Distance(hit.point, gtPos) < 0.2f)
					return true;
